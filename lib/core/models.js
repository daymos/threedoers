(function() {
  var Comment, ObjectId, PROJECT_BOUNDARIES, PROJECT_COLORS, PROJECT_DENSITIES, PROJECT_MATERIALS, PROJECT_STATUSES, STLProject, Schema, Subscription, decimal, gridfs, inflection, mailer, models, modelsNot, mongoose, settings,
  ...
  STLProject.pre('save', function(next) {
    var now, that, _ref;
    this.editable = (_ref = this.status) === PROJECT_STATUSES.PROCESSED[0] || _ref === PROJECT_STATUSES.PRINT_REVIEW[0];
    now = new Date();
    if (!this.createdAt) {
      this.createdAt = now;
    }
