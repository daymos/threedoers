(function() {
  module.exports = function(app, io) {
    var STLStats, decorators, exec, fs, gridfs, logger, mailer, models, processVolumeWeight, settings, utils;
    fs = require('fs');
      ...
      if (socket.handshake.query.project != null) {
        return models.STLProject.findOne({
          ...
          if (doc) {
            ...
            return io.of('/project')["in"](doc._id.toHexString()).emit('update', doc._doc);
          } else {
    ...
    return processVolumeWeight = function(doc) {
      return exec("" + settings.python.bin + " " + settings.python.path + " " + settings.upload.to + doc.file + " -d " + doc.density, function(err, stdout, stderr) {
        ...
        if (!err && !stderr) {
          try {
            ...
            doc.price = (doc.volume * 1.01 * doc.density * 0.03) + 5;
            doc.bad = false;
            doc.save();
          } catch (_error) {
            e = _error;
