(function() {
  module.exports = function(app, io) {
    var Paypal, STLStats, auth, calculateOrderPrice, decimal, decorators, designModels, exec, fs, gridfs, handleDirection, logger, mailer, models, processVolumeWeight, request, requestShippingRate, settings, shippo, utils;
    fs = require('fs');
      ...
      return models.STLProject.findOne(filterDict).exec().then(function(doc) {
        if (doc) {
          ...
          if (doc.order && doc.order.printer) {
            return auth.User.findOne(doc.order.printer).exec().then(function(printer) {
            ...
            return shippo.shipment.rates(shipping.object_id, 'EUR').then(function(rates) {
              ...
              if (rates.count && rates.count > 0) {
                ...
                for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                  ...
                  if (rate_tmp.object_purpose === "PURCHASE" && price > price_tmp && price_tmp > 0) {
                    rate = rate_tmp;
                    price = price_tmp;
                  }
                }
      ...
      return models.STLProject.findOne({
        ...
        if (doc && doc.status >= models.PROJECT_STATUSES.PRINT_REVIEW[0]) {
          if (req.body.message) {
            ...
            return auth.User.where('_id')["in"]([req.user.id, doc.printer]).exec().then(function(docs) {
              if (docs.length) {
          ...
          return auth.User.findOne({
            ...
            if (printer) {
              if (printer.printerAddress) {
                ...
                return auth.User.where('id')["in"]([req.user.id, doc.printer]).exec().then(function(docs) {
                  var user, _i, _len, _results;
                  if (docs.length) {
          ...
          return auth.User.findOne(doc.user).exec(function(err, user) {
            ...
            if (user) {
              ...
              if (!(req.user.printerAddress || req.user.printerAddress.object_id)) {
                res.json({
                  msg: "You don't have a shipping address please ask him to add an address and activate it."
                }, 400);
                return;
              }
              if (user.mailNotification) {
    ...
    app.post('/goshippo-webhook/', function(req, res) {
    ...
    app.post('/cron/update-rates', function(req, res) {
      models.STLProject.find({
        ...
        for (_i = 0, _len = docs.length; _i < _len; _i++) {
          project = docs[_i];
          _results.push(auth.User.findOne(project.user).exec().then(function(user) {
      ...
      return exec("" + settings.python.bin + " " + settings.python.path + " " + settings.upload.to + doc.file + " -d " + doc.density, function(err, stdout, stderr) {
        ...
        if (!err && !stderr) {
          try {
            ...
            console.log(result);
            material_price = doc.material === 'ABS' ? 0.5 : 0.5 * 1.1;
            density = doc.density;
            fixed_cost = 8;
    ...
    return calculateOrderPrice = function(basePrice, ammount) {
      return decimal.fromNumber((basePrice * ammount) - (7 * (ammount - 1)), 2);
    };
  };
