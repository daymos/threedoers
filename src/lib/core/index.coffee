...
module.exports = (app, io) ->
  ...
  app.get '/project/:id', (req, res, next) ->
    ...
    if req.user
      notificationModels.Notification.update({recipient: req.user.id, relatedObject: req.params.id, type:1, read: false}, {read: true}).exec()
    ...
    models.STLProject.findOne({_id: req.params.id, $or: [{user: req.user.id}, {'order.printer': req.user.id}]}).exec().then( (doc) ->
      if doc and doc.status >= models.PROJECT_STATUSES.PRINT_REVIEW[0]  # test if comments allowed
        if req.body.message
          ...
          recipient = if req.user._id.equals(doc.user) then doc.order.printer else doc.user
          notificationModels.Notification.find({recipient: recipient, relatedObject: doc.id, type:1, read: false}).exec().then (notifications) ->
